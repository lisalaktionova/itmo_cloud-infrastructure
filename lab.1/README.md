...
Выбранный диапазон не пересекается с сетями, уже зафиксированными в инфраструктуре компании:
...
## 2. Таблица подсетей
Во всех таблицах присутствует системный маршрут `10.100.0.0/16 → local`, обеспечивающий связь между подсетями VPC.
...
- **NAT Gateway** — резервный и общий исходящий маршрут для обновлений ОС и контейнеров, резервного копирования во внешнее S3-хранилище.
...
## 6.3. Резервирование каналов с on-premise
...
Механизм переключения — BGP. В штатном режиме маршрутизация предпочитает Direct Connect. После обнаружения отказа и исключения основного маршрута трафик направляется через работающий VPN. Так on-premise-сервисы (например, складская система или бухгалтерия) остаются доступны даже при обрыве основного канала.
